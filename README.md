...
2. Allow the user to load a `Makefile` by entering its URL into a textbox
3. Allow the user to upload a `Makefile` by browsing for it on their computer
4. Expose the intermediate Mermaid diagram code
5. Test SVG downloading on mobile devices
6. Allow user to choose the graph direction (`TB` versus `LR`)
7. Maybe: Add SVG pan and zoom
8. Maybe: Store recently-viewed Makefiles in browser storage
